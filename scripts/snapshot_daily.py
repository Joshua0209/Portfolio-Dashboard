...
# Mirror app/__init__.py:_load_env so `python scripts/snapshot_daily.py`
# sees Shioaji creds (and other env vars) without the operator having to
# `set -a; source .env` first. override=False keeps real shell env wins.
try:
...
def _run_overlay_safe(
    store: DailyStore, portfolio: dict, today: str,
    ...
    and projects the pre-pulled tuple. snapshot_daily.run() uses this to
    pull SDK sources once, fetch prices for newly-discovered symbols,
    then run the projection — single SDK roundtrip per refresh.
    """
    overlay_summary: dict[str, Any] = {
        "overlay_trades": 0, "dates_written": 0, "skipped_reason": "no_gap",
    }
    try:
        ...
        overlay_summary = {
            "overlay_trades": 0, "dates_written": 0, "skipped_reason": "exception",
        }
    return overlay_summary


def _fetch_overlay_symbol_prices(
    store: DailyStore, portfolio: dict, end: str,
    pdf_symbols: set[str],
    ...
    """
    from app import backfill_runner, trade_overlay
    from app.shioaji_client import ShioajiClient
    ...
    log.info(
        ...
        len(overlay_symbols), len(pdf_symbols), len(extra_symbols),
...
def run(store: DailyStore, portfolio: dict) -> dict[str, Any]:
    ...
    # the merge step just wrote. Single source of truth for equity_twd.
    backfill_runner._derive_positions_and_portfolio(store, portfolio)
