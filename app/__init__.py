...
def create_app(data_path: Path | str | None = None) -> Flask:
    ...
    # Treat empty `DAILY_DB_PATH=` (the .env.example default) as unset —
    # otherwise os.environ.get returns "" and Path("") resolves to ".",
    # which sqlite3.connect rejects with "unable to open database file".
    daily_db_override = os.environ.get("DAILY_DB_PATH") or None
    daily_db_path = (
        Path(daily_db_override) if daily_db_override
        else project_root / "data" / "dashboard.db"
    )
