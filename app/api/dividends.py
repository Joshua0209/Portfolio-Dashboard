"""Dividends — TW + foreign dividend events plus rebates.

Source-of-truth is `summary.dividends` (bank-derived per-event records). Each
record carries date, ticker, currency, local + TWD amount. Rebates come in
as a separate stream because they are broker promotional credits, not
shareholder distributions; the page can show both with a clear label.
"""
...
def dividends():
    ...
    # Rebates: 折讓金 — broker promotional credits, shown separately.
    ...
    # Trailing-12-month yield estimate: TTM dividend / current invested cost.
...
def _months_ago(month_str: str | None, n: int) -> str:
    """Return a 'YYYY/MM/01' lower-bound for the trailing n-month window
    starting from `month_str` ('YYYY-MM'). Used to filter dividend rows by date."""
    if not month_str:
        return "0000/00/00"
    y, m = month_str.split("-")
    y, m = int(y), int(m)
