...
def tax():
    ...
    Unrealized P&L sources from current_holdings() — see
    app/holdings_today.py for the warm/cold resolution rules. Same
    function feeds /api/summary and /api/holdings/current, so the
    three pages always agree on unrealized totals.
    """
    s = store()
    ...
    current = current_holdings(s, daily_store()) if s.months else []
