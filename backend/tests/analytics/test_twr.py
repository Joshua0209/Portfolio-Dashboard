...
class TestDayWeightedOutOfBounds:
    ...
    def test_flow_after_period_end_clamps_to_zero_weight(self):
        # A flow dated after period_end would give negative weight without
    ...
    def test_multi_flow_day_weighted_weighted_sum(self):
        """Multiple flows in one period: the day-weighted denominator is
        the sum of each flow's weight*amount, not a single mid-month factor.

        Period: 2026-05-01 to 2026-05-31 (D = 30 days).
        Flows:
          Flow A: +200 000 on 2026-05-06 → elapsed=5, weight=(30-5)/30=25/30
          Flow B: +100 000 on 2026-05-21 → elapsed=20, weight=(30-20)/30=10/30

        numerator = V_end - V_start - (200k+100k)
                  = 1.5M - 1M - 300K = 200K
        denominator = 1M + (25/30)*200K + (10/30)*100K
                    = 1M + 166666.67 + 33333.33
                    = 1 200 000
        r = 200K / 1 200 000 = 1/6
        """
        r = modified_dietz(
            start_equity=Money(Decimal("1000000"), "TWD"),
            end_equity=Money(Decimal("1500000"), "TWD"),
            cashflows=[
                Cashflow(
                    date=date(2026, 5, 6),
                    amount=Money(Decimal("200000"), "TWD"),
                    kind=CashflowKind.DEPOSIT,
                ),
                Cashflow(
                    date=date(2026, 5, 21),
        ...
        expected = Decimal("200000") / Decimal("1200000")
        assert abs(r - expected) < Decimal("1E-9")
