...
def test_tw_falls_back_to_otc_when_listed_empty(
    ...
    monkeypatch.setattr("invest.prices.sources.yfinance_fetch_prices", fake)
...
def test_market_verdict_unknown_when_neither_responds(
    ...
    monkeypatch.setattr("invest.prices.sources.yfinance_fetch_prices", lambda *_: [])
...
def test_cached_twse_symbol_skips_tpex_suffix_on_rerun(
    ...
    monkeypatch.setattr("invest.prices.sources.yfinance_fetch_prices", fake1)
    get_prices("2330", "TWD", "2025-06-01", "2025-06-30", store=store)
    ...
    monkeypatch.setattr("invest.prices.sources.yfinance_fetch_prices", fake2)
...
def test_cached_tpex_symbol_skips_listed_suffix_on_rerun(
    ...
    monkeypatch.setattr("invest.prices.sources.yfinance_fetch_prices", wrapped)
