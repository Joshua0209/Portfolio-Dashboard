...
from invest.prices.sources import get_fx_rates, get_prices
...
def test_get_prices_usd_routes_to_yfinance(monkeypatch) -> None:
    ...
    monkeypatch.setattr("invest.prices.sources.yfinance_fetch_prices", fake_yf_prices)
...
def test_get_fx_rates_returns_tagged_rows(monkeypatch) -> None:
    monkeypatch.setattr(
        "invest.prices.sources.yfinance_fetch_fx",
        lambda ccy, st, ed: [
