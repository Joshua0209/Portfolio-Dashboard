...
def _daily_drawdown_curve(daily: DailyStore) -> tuple[list[dict] | None, float | None]:
    rows = daily.get_drawdown_series()
    if not rows:
        return None, None
    curve = [
        {
...
def risk(
    ...
    holdings = current_holdings(s, daily)
    ...
    daily_episodes = None
    if use_daily:
        daily_dd_curve, daily_current_dd = _daily_drawdown_curve(daily)
        if daily_dd_curve:
