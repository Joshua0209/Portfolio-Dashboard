...
-- Persisted post-PDF trades discovered via the Shioaji overlay.
-- Mirrors the PDF trade shape (date/code/side/qty/price/fee/tax/gross/net)
-- so /api/transactions can UNION both sources without per-source branching.
-- net_twd is approximated per the formulas in trade_overlay._compute_overlay_net_twd:
--   TW buy:  net_twd = -(qty * price * (1 + fee_pct))      where fee_pct = 0.001425
--   TW sell: net_twd = +(qty * price * (1 - fee_pct - tax_pct))
--                                                         where tax_pct = 0.003
--   Foreign: net_twd = ±qty * price * fx_to_twd            (no fee, no tax)
-- Margin (融資) trades are treated as cash for the overlay net_twd math —
-- the broker loan portion isn't recoverable from the SDK without separate
-- account-loan queries.
--
-- PRIMARY KEY uses (date, code, side, qty_int) where qty_int = round(qty).
-- This matches trade_overlay._trade_dedup_key so re-running merge() against
-- the same SDK state UPSERTs idempotently rather than appending dupes.
CREATE TABLE IF NOT EXISTS trades_overlay (
    date           TEXT NOT NULL,
    code           TEXT NOT NULL,
    side           TEXT NOT NULL,
    qty_int        INTEGER NOT NULL,
    qty            REAL NOT NULL,
    price          REAL NOT NULL,
    fee_twd        REAL NOT NULL,
    tax_twd        REAL NOT NULL,
    gross_twd      REAL NOT NULL,
    ...
    PRIMARY KEY (date, code, side, qty_int)
);
CREATE INDEX IF NOT EXISTS idx_trades_overlay_date ON trades_overlay(date);
"""
