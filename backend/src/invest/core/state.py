"""Daily-resolution backfill state machine.
...
Three states drive the warming-up UX:
    ...
    FAILED        → backfill thread crashed; daily endpoints return 503
                    ...
                    via the Developer Tools accordion.
...
Lives in `invest.core` (not `invest.jobs`) because both the HTTP layer
(`/api/health`, daily-state gate in today/daily routers) and the jobs
layer (backfill writes; snapshot reads) need it. Putting it in the
writer's package would force the HTTP routers to import from `jobs/`
just to read state.
"""
...
class BackfillState:
    ...
    def __init__(self) -> None:
        ...
        # work is in flight. This way the daily-state gate doesn't 202 in
        # cases where no work is happening (operator ran the snapshot CLI
        # manually, or BACKFILL_ON_STARTUP is off entirely).
