...
def test_summary_kpis_untouched_without_daily(app_empty_daily):
    ...
    kpis = data["kpis"]
    # Values still match the parser-provided 1_050_000 / 250_000.
    assert kpis["real_now_twd"] == pytest.approx(1_050_000)
    assert kpis["profit_twd"] == pytest.approx(250_000)
    # Under the unified-source contract (current_holdings()), kpis
    # always carry repriced_holdings_count — 0 indicates cold path
    # (PDF month-end), >0 indicates warm path (positions_daily). The
    # frontend reads the count to decide whether to show the
    # "repriced today" badge.
    assert kpis["repriced_holdings_count"] == 0
