...
# --- record_event() audit-hook helper (Phase 11 Path A) -------------------
#
# Plan §"Audit hook (decision #1, option C)" calls for the merge layer to
# fire reconcile events when the SDK's buy-leg count diverges from the
# PDF parser's trades. record_event() is the persistence primitive that
# the merge layer (and any future audit caller) uses. The existing
# reconcile_events schema is REUSED — no migration. event_type and
# detail are encoded inside the existing diff_summary JSON envelope.


def test_record_event_inserts_row_and_returns_id(store):
    """record_event(store, event_type, detail) → event_id > 0.

    Round-trips event_type and detail through diff_summary JSON.
    """
    detail = {
        "pair_id": 101, "code": "7769", "sell_date": "2026-04-15",
        "sdk_leg_count": 5, "pdf_trade_count": 3,
    }
    event_id = reconcile.record_event(
        store,
    ...
    assert len(open_events) == 1
    payload = json.loads(open_events[0]["diff_summary"])
    assert payload["event_type"] == "broker_pdf_buy_leg_mismatch"
    assert payload["detail"]["sdk_leg_count"] == 5
    assert payload["detail"]["pair_id"] == 101


def test_record_event_accepts_optional_pdf_month(store):
    """The pdf_month column is part of the schema. Audit-hook events
    optionally carry it (e.g., the sell-date's month) so banners can
    deep-link by month. Default '' is fine for non-month-bound events."""
    event_id = reconcile.record_event(
        store,
        event_type="broker_pdf_buy_leg_mismatch",
        detail={"pair_id": 200},
    ...
    with store.connect_ro() as conn:
        row = conn.execute(
            "SELECT pdf_month FROM reconcile_events WHERE id = ?", (event_id,)
        ).fetchone()
    assert row["pdf_month"] == "2026-04"


def test_record_event_default_pdf_month_is_empty_string(store):
    """When pdf_month not supplied, store '' (NOT NULL column)."""
    event_id = reconcile.record_event(
        store,
        event_type="degenerate_pair_legs_unrecoverable",
        detail={"pair_ids": [301, 302]},
    )
    with store.connect_ro() as conn:
        row = conn.execute(
            "SELECT pdf_month FROM reconcile_events WHERE id = ?", (event_id,)
        ).fetchone()
    assert row["pdf_month"] == ""


def test_record_event_dismiss_works(store):
    """dismiss_event() must work uniformly for both event flavors —
    PDF-vs-overlay diffs (existing) AND audit-hook events (new)."""
    event_id = reconcile.record_event(
        store,
        event_type="broker_pdf_buy_leg_mismatch",
        detail={"pair_id": 400},
...
def test_record_event_coexists_with_run_for_month_events(store, portfolio_with_march_trade):
    """Both event types share the table. get_open_events surfaces both."""
    # 1. Audit event
    reconcile.record_event(
        store, "broker_pdf_buy_leg_mismatch",
        detail={"pair_id": 500, "code": "2330"},
    )
    # 2. Existing PDF-vs-overlay diff event
    overlay = lambda s, e: [
        {"date": "2026-03-20", "code": "0050", "side": "普買",
         "qty": 2000, "price": 195.0},
    ]
    reconcile.run_for_month(
        store, portfolio_with_march_trade, "2026-03", overlay_client=overlay
    )
    ...
    types = []
    for e in events:
        payload = json.loads(e["diff_summary"])
        # Both shapes coexist: audit events have event_type;
        # legacy diff events do not (or carry a default).
        types.append(payload.get("event_type") or "pdf_overlay_diff")
    assert set(types) == {"broker_pdf_buy_leg_mismatch", "pdf_overlay_diff"}
